...
const Layout = (props) => {
  ...
  // const [maintainceMode, setMaintainceMode] = useState(false);
  ...
  useEffect(() => {
    ...
    /** checking the current zone and validate
     * redirect to correct zone pm70-06-01-22
     */
    // setTimeout(() => {
    //     getDetectedLocation().then((updatedZone) => {
    //         if (zone !== updatedZone.code) {
    //             router.push({
    //                 pathname: router.pathname,
    //                 query: router.query,
    //             }, (router.asPath.replace(`/${router.query.zone_lang}`, `/${updatedZone.defaultLocale}-${updatedZone.code}`)), { shallow: true });
    //             console.log('not matched zone');
    //         }
    //     });
    // }, 1000);
  }, []);
  ...
  // const [getClientIp, { data: clientIp }] = useManualQuery(getClientIpAPI.loc.source.body, {
  //     fetchPolicy: 'network-only',
  // });

  // const [getWhiteListIp, { data: dataIPList }] = useManualQuery(whiteListedIP.loc.source.body, {
  //     fetchPolicy: 'network-only',
  // });
  ...
  //         setTimeout( () => {
  //             var noscr = document.getElementsByTagName('noscript');
  //             console.log(noscr);
  //             for(var yh = 0; yh<20; yh++) {
  //                 if (document.getElementsByTagName('noscript')[yh]) {
  //                     console.log('Koustov Ganguluy', document.getElementsByTagName('noscript')[yh]);
  //                     document.getElementsByTagName('noscript')[yh].remove();
  //                 }
  //             }
  //         }, 50000)
  //     } catch (error) {
  ...
  /* const { loading: loadingMaintenanceNotification, data: dataMaintenanceNotification } = useQuery(maintenanceBanner.loc.source.body,
        {
            operationName: {
                clientName: "graphCms"
            },
    ...
    if (loadingMaintenanceNotification) return null;
    if (dataMaintenanceNotification) {
        setMaintenanceNotification(dataMaintenanceNotification.maintenances[0].maintenanceNotificationEnable);
    } */
